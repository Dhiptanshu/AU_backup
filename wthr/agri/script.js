const API_BASE = "http://127.0.0.1:8001/api/agri";
...
function applyFilter(crop) {
    ...
    document.querySelectorAll('.filter-btn').forEach(btn => {
        if (btn.innerText.includes(crop) || (crop === 'All' && btn.innerText === 'All')) {
...
async function handleVerification(event) {
    ...
    try {
        const res = await fetch(`${API_BASE}/batches/`, { method: 'POST', body: formData });
        if (!res.ok) throw new Error("Upload Failed");
        const data = await res.json();

        statusDiv.innerHTML = `<span style="color:orange">📡 Minting...</span>`;
...
async function fetchData() {
    ...
    try {
        ...
        const filteredData = currentFilter === 'All' ? data : data.filter(b => b.crop_name === currentFilter);

        updateStats(filteredData);

        container.innerHTML = "";
        if (filteredData.length === 0) {
            container.innerHTML = '<div style="text-align:center; padding:20px; color:#64748b">No shipments found.</div>';
            return;
        }
...
function updateStats(data) {
    const totalKg = data.reduce((sum, b) => sum + b.quantity_kg, 0);
    const verifiedKg = data.filter(b => b.status === 'VERIFIED').reduce((sum, b) => sum + b.quantity_kg, 0);

    document.getElementById('kpi-stock').innerText = (totalKg).toLocaleString() + " kg";
    document.getElementById('kpi-trucks').innerText = data.length;
    document.getElementById('kpi-verified').innerText = (verifiedKg).toLocaleString() + " kg";
}
    ...
    try {
        ...
        container.innerHTML = data.commodities.map(c => `
            <div class="price-card" style="border-left: 4px solid ${c.trend === 'up' ? '#ef4444' : '#10b981'};">
                ...
                <div style="font-size:1.4rem; font-weight:800; color:#f8fafc; margin:5px 0;">
                    ₹${c.modal_price} <span style="font-size:1rem; font-weight:normal; color:#94a3b8;">/kg</span>
                </div>
